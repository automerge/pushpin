...
export function cardTextResized(state, { id, height }) {
  const newBoard = changeBoard(state, (b) => {
    ...
    // The `+ (GRID_SIZE / 2)` forces the snap to round the height up, which
    // ensures that text doesn't stick out of the bottom of card with
    // contracted height.
    card.height = snapMeasureToGrid(height + (GRID_SIZE / 2))
  })
  return { ...state, board: newBoard }
}
